...
namespace xn
{
    ...
    internal static class TianyunziSpritePatch
    {
        ...
        {
            int isTYZ = 0;
            xn.access.ActorAccess.GetData(__instance).get(KEY_FLAG_TYZ, out isTYZ, 0);
            if (isTYZ != 1) return;
            if (s_tianyunzi == null)
            {
                s_tianyunzi = LoadTyz();
            ...
            xn.access.ActorAccess.GetData(__instance).get(KEY_BASE_SCALEX, out baseX, 0f);
            xn.access.ActorAccess.GetData(__instance).get(KEY_BASE_SCALEY, out baseY, 0f);
            if (baseX <= 0f || baseY <= 0f)
            {
                var sc0 = __instance.current_scale;
                baseX = sc0.x; baseY = sc0.y;
                xn.access.ActorAccess.GetData(__instance).set(KEY_BASE_SCALEX, baseX);
                xn.access.ActorAccess.GetData(__instance).set(KEY_BASE_SCALEY, baseY);
            }
            ...
            if (newH > 0f && oldH > 0f)
            {
                ...
                float extra = 1f;
                xn.access.ActorAccess.GetData(__instance).get(KEY_EXTRA_PCT, out extra, 1f);
